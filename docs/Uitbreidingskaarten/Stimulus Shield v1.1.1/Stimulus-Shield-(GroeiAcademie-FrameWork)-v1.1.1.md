...
> Status: werkdocument v1.1.1. ADS1115 blijft ondersteund als hardwarematig geteste uitbreiding.
> Deze versie bevat volledig de v1.1.0-hardware en documenteert aanvullend de HX1838 IR Receiver in het v1.1.1-shieldschema.

> **Gezaghebbende hardwarebron**: `Schematic_Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1_2026-08-24` (shield-PCB). Dit document beschrijft H6 conform die shield-uitvoering: 4x2 jumpers voor directe Arduino-ADC. De ADDR-keuze gebeurt via SW1. De voedingskeuze voor het FSR/ADS1115-pad gebeurt via H7, de 3-pin jumper 5V/3V3. H8 is in dit v1.1.1-schema de 1x7 TFTSPI-displayconnector. Figuur 1 hieronder (het generieke connectorschema uit een vroegere ontwerpfase) is een **historische/conceptuele referentie — niet gebouwd, niet onderhouden**. Het toont de redenering achter de ADS1115-uitbreiding, maar wijkt af van de effectief te bouwen print en mag niet als bouwinstructie gebruikt worden.
...
Figuur 1 — historisch/conceptueel schema: niet opgenomen in deze release; gebruik voor bouw en validatie uitsluitend de actuele v1.1.1-tekeningset.
...
De ADDR-adreskeuze gebeurt in het v1.1.1-shieldschema via SW1. Er mag altijd maar één SW1-schakelaar tegelijk ON staan. Voor Stimulus is de standaard ADDR naar GND, dus adres 0x48.
...
In dit deel worden de connectorreferenties uit het schema expliciet gebruikt. Zo blijft de Markdown rechtstreeks bruikbaar naast het schema. In v1.1.1 zijn er vier onafhankelijke keuzes: het ADC-pad, de I2C-niveauconversie, de TFTSPI-niveauconversie en het input-pad. Voor Input blijft de bestaande 1x4-keymatrix rechtstreeks via H1/D2-D5 beschikbaar, blijft de PCF8574-uitbreiding via de gedeelde I2C-bus beschikbaar en wordt de HX1838 IR Receiver toegevoegd via D12.
...
| U10 | HX1838 IR Receiver; DATA is verbonden met Arduino D12 |
...
H5 is de centrale 10-polige Dupont-connector voor het ADS1115-pad en de vier analoge sensorlijnen. H6 is een afzonderlijk 4x2-jumperveld. Met vier geplaatste jumpers verbindt H6 H5 pin 7 tot en met pin 10 met Arduino A0 tot en met A3. H7, de 3-pin jumper 5V/3V3, is in v1.1.1 uitsluitend de voedingskeuze voor het FSR/ADS1115-pad. SW1 is uitsluitend de ADDR-keuze voor de ADS1115-adreskeuze. H8 is de TFTSPI-displayconnector.
...
Het v1.1.1-schema laat de bestaande directe 1x4-keymatrix op H1/D2-D5 en de PCF8574-uitbreiding uit v1.1.0 intact en voegt daarnaast een HX1838 IR Receiver toe.
...
De directe keymatrix en de PCF8574-route blijven beschikbaar voor fysieke keypads. De HX1838 vormt daarnaast een IR-invoerroute. De softwarekeuze gebeurt via `INPUT_KANAAL_CONFIG`; `KEYPAD_TYPE` beschrijft het aangesloten keypad. De HX1838 DATA-lijn is op het shield verbonden met Arduino D12. Ieder I2C-apparaat op dezelfde bus moet een uniek adres hebben.
...
De HX1838 is in v1.1.1 als extra Input-hardware toegevoegd. De bestaande hardware uit v1.1.0 blijft ongewijzigd beschikbaar.

| Onderdeel | Functie |
|---|---|
| U10 | HX1838 IR Receiver |
| H13 | 1x3 Dupont-connector voor GND, VCC en DATA |
| DATA | Verbonden met Arduino D12 |
...
- HX1838: controleer dat U10 via H13 op GND, VCC en DATA aangesloten is en dat DATA naar Arduino D12 loopt.
...
## 20. Actuele bestandenset v1.1.1

De tekeningset en documentatie voor deze hardwareversie zijn:

- `SCH_Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1_2026-08-24.json`;
- `Schematic_Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1_2026-08-24.pdf`;
- `Schematic_Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1_2026-08-24.png`;
- `Schematic_Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1_2026-08-24.svg`;
- `Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1.md`;
- `Handleiding-Stimulus-Shield-(GroeiAcademie-FrameWork)-v1.1.1.md`.
...
### Toegevoegd in v1.1.1

Stimulus Shield v1.1.1 bevat de volledige hardware van v1.1.0 en voegt daaraan toe:

- **U10: HX1838 IR Receiver**;
- **H13: 1x3 Dupont-connector** voor GND, VCC en DATA;
- **DATA naar Arduino D12**.

De bestaande v1.1.0-hardware, connectoren en bestukkingskeuzes blijven behouden.
...
### Experimenteel toegevoegd in v1.1.0

Onderstaande drie boards zijn vanaf v1.1.0 opgenomen als **experimentele** ondersteuning. Ze zijn nog niet fysiek hardwarematig gevalideerd; zie `docs/HARDWARE_SUPPORT.md` voor de openstaande punten per board.
