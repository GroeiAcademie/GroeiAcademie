...
Figuur 1 — historisch/conceptueel schema: niet opgenomen in deze release; gebruik voor bouw en validatie uitsluitend de actuele v1.1.0-tekeningset.
...
De ADDR-adreskeuze gebeurt in het v1.1.0-shieldschema via SW1. Er mag altijd maar één SW1-schakelaar tegelijk ON staan. Voor Stimulus is de standaard ADDR naar GND, dus adres 0x48.
...
In dit deel worden de connectorreferenties uit het schema expliciet gebruikt. Zo blijft de Markdown rechtstreeks bruikbaar naast het schema. In v1.1.0 zijn er vier onafhankelijke keuzes: het ADC-pad, de I2C-niveauconversie, de TFTSPI-niveauconversie en het input-pad. Voor Input kan de bestaande 1x4-keymatrix rechtstreeks via H1/D2-D5 worden gebruikt of kan een PCF8574-uitbreiding via de gedeelde I2C-bus worden aangesloten.
...
H5 is de centrale 10-polige Dupont-connector voor het ADS1115-pad en de vier analoge sensorlijnen. H6 is een afzonderlijk 4x2-jumperveld. Met vier geplaatste jumpers verbindt H6 H5 pin 7 tot en met pin 10 met Arduino A0 tot en met A3. H7, de 3-pin jumper 5V/3V3, is in v1.1.0 uitsluitend de voedingskeuze voor het FSR/ADS1115-pad. SW1 is uitsluitend de ADDR-keuze voor de ADS1115-adreskeuze. H8 is de TFTSPI-displayconnector.
...
### 6.3 Input-pad: direct of via PCF8574

Het v1.1.0-schema laat de bestaande directe 1x4-keymatrix op H1/D2-D5 intact en voegt daarnaast een optionele PCF8574-uitbreiding toe.
...
De directe keymatrix en de PCF8574-route zijn alternatieve manieren om een fysiek keypad uit te lezen. De softwarekeuze gebeurt via `INPUT_KANAAL_CONFIG`; `KEYPAD_TYPE` beschrijft het aangesloten keypad. Ieder I2C-apparaat op dezelfde bus moet een uniek adres hebben.
...
## 20. Actuele bestandenset v1.1.0
