...
bool ScreenResourcesAanmeldenOpGedeeldeBus() {
  ...
  if (!UARTAanmeldenOpGedeeldeBus(GedeeldeBusComponent::SERIAL_OUTPUT, true)) return false;
#endif
...
// ADC_ADS1115 heeft een eigen, gedeelde I2C-bus (het adres zelf); ADC_NATIVE heeft dat niet
// en meldt in plaats daarvan elke effectief gebruikte sensorpin apart aan.
bool StimulusResourcesAanmeldenOpGedeeldeBus() {
#if ADC_BACKEND == ADC_BACKEND_ADS1115
  if (!I2CAanmeldenOpGedeeldeBus(GedeeldeBusComponent::ADC_ADS1115, I2C_ADDRESS_ADS1115, HardwareResourcePin::SDA, HardwareResourcePin::SCL, true)) return false;
#else
#if (AANTAL_SENSOREN_AANWEZIG == 4)
  if (!AanmeldenHardwareResourcesOpGedeeldeBus(GedeeldeBusComponent::ADC_NATIVE, HardwareResourceType::ADC, {PIN_SENSOR_1, PIN_SENSOR_2, PIN_SENSOR_3, PIN_SENSOR_4}, HardwareResourceToegang::EXCLUSIEF, true)) return false;
#else
  if (!AanmeldenHardwareResourcesOpGedeeldeBus(GedeeldeBusComponent::ADC_NATIVE, HardwareResourceType::ADC, {PIN_SENSOR_1, PIN_SENSOR_2}, HardwareResourceToegang::EXCLUSIEF, true)) return false;
#endif
#endif
  return true;
}
...
bool InputResourcesAanmeldenOpGedeeldeBus() {
  ...
  if (!AanmeldenHardwareResourcesOpGedeeldeBus(GedeeldeBusComponent::INPUT_DIGITAL, HardwareResourceType::GPIO, {PIN_TOETS_1, PIN_TOETS_2, PIN_TOETS_3, PIN_TOETS_4}, HardwareResourceToegang::EXCLUSIEF, true)) return false;
#endif
...
void setup() {
  ...
  StimulusResourcesAanmeldenOpGedeeldeBus();
  InputResourcesAanmeldenOpGedeeldeBus();

  // FASE 2 en 3.
  bool magInpluggen = AanmeldingenInpluggenOpGedeeldeBus();
  if (!magInpluggen) {
    PrintToScreen("GedeeldeBus", "RESOURCECONFLICT", LCD_LEESTIJD_MEDEDELING_KORT_MS);
    return;
  }
